...
def venvcmd(cmd, shell=True, user="spreadband", pty=False, subdir=""):
...
def prodenv():
    ...
    commonenv()
    env.venvname = "preview.spreadband.com"
    env.wsginame = "preview.wsgi"
...
def _updatemaincode():
    ...
    with cd(env.venvfullpath + '/%(projectname)s/' % env):
        sudo('hg pull -u', user="spreadband")
