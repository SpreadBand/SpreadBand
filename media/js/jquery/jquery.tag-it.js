(function($) {
	 ...
	 // add existing tags
	     .children("li")
             .each(function() {
			   ...
			   $.fn.tagit.create_tag($(this).html(), $(this).attr("class"));
			   $(this).remove();
		       }
		   });
	 ...
	 tagInput
	     ...
	     .keypress(function(event) {
			   ...
			   if (
			       ...
			       $.fn.tagit.create_tag(tagInput.val().replace(/^"|"$|,+$/g, "").trim());
			   }
	 ...
	 if (options.availableTags || options.tagSource) {
	     tagInput.autocomplete({
					   ...
					   $.fn.tagit.create_tag(ui.item.value);
	 ...
	 $.fn.tagit.create_tag = function create_tag(value, additionalClass) {
	     // Cleaning the input.
	     tagInput.val("");
		 ...
		 .append("<input type=\"hidden\" style=\"display:none;\" value=\"" + value + "\" name=\"" + settings.itemName + "\">");
